...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function inicializarApp() {
        ...
        try {
            ...
            // 1. Determinar y cargar el idioma
            const lang = urlParams.get('lang') || 'es'; // Detecta el idioma
            await loadTranslations(lang); // Carga el diccionario
            
            // 2. Cargar la configuración de la obra
            ...
            // 3. Aplicar traducciones estáticas y determinar la vista inicial
            applyStaticTranslations(lang);
            actualizarBotonDashboard();

            // 4. Determinar la vista inicial a mostrar
            const vistaInicial = urlParams.get('vista');
            if (vistaInicial === 'dashboard') {
                // Si la URL pide el dashboard, lo mostramos directamente
                mostrarVistaDashboard();
            } else {
                // Si no, cargamos los planos como antes
    ...
    function toggleVistaPrincipal() { 
        // Si estamos en la vista de planos, vamos al dashboard. Si no, vamos a los planos.
        if (document.getElementById('vista-unificada-container').style.display === 'flex') {
            mostrarVistaDashboard();
        } else {
            mostrarVistaPlanos();
        }
    ...
    function mostrarVistaPlanos() {
        ...
        todosSensoresContainer.style.display = 'none'; // Ocultar la nueva vista
    ...
    function mostrarVistaDashboard() {
        ...
        if (tituloHeaderDinamico) tituloHeaderDinamico.textContent = "Dashboard";
        if (tituloPaginaDinamico) tituloPaginaDinamico.textContent = configActualEscena.nombreParaTitulo;
        if (botonToggleVista) {
            // Este botón ahora siempre te llevará de vuelta a los planos
            botonToggleVista.innerHTML = `<i class="fas fa-ruler-combined"></i> Ver Planos`;
    ...
    function mostrarVistaTodosSensores() {
        document.getElementById('vista-unificada-container').style.display = 'none';
        dashboardContainer.style.display = 'none';
        todosSensoresContainer.style.display = 'flex'; // Mostrar la nueva vista

        vistaActualEsDashboard = true; // La consideramos una vista de "dashboard"
        if (tituloHeaderDinamico) tituloHeaderDinamico.textContent = "Todos los Sensores";
        if (tituloPaginaDinamico) tituloPaginaDinamico.textContent = configActualEscena.nombreParaTitulo;
        
        // Llamamos a una nueva función que se encargará de pintar todos los gráficos
    ...
    async function fetchAndDrawCharts() {
        ...
        const existingMessage = chartsGrid.querySelector('.loading-message') || chartsGrid.querySelector('.error');
        if (existingMessage) existingMessage.remove();

        const loadingMessage = document.createElement('p'); loadingMessage.className = 'loading-message';
        loadingMessage.textContent = 'Cargando gráficos por defecto...'; chartsGrid.appendChild(loadingMessage);
        ...
        try {
            ...
            if (response.status === 401) {
                // Si la sesión ha expirado, redirigimos al login.
            ...
            if (!response.ok) {
                // Para otros errores, lanzamos una excepción normal.
    ...
    function createChartCard(title, data, isCustom) {
        ...
        for (const sensorId in data) {
            ...
            if (!variableToAxisId.has(variableName)) {
                ...
                if (translatedVariableName.toLowerCase().includes('desplazamiento')) {
                    yAxes[yAxisID].min = -20;
                    yAxes[yAxisID].max = 20;
                } else if (translatedVariableName.toLowerCase().includes('inclinacion')) {
                    yAxes[yAxisID].min = -10;
                    yAxes[yAxisID].max = 10;
                } else if (translatedVariableName.toLowerCase().includes('temperatura')) {
                    yAxes[yAxisID].min = 10;
                    yAxes[yAxisID].max = 30;
                } else if (translatedVariableName.toLowerCase().includes('humedad')) {
                    yAxes[yAxisID].min = 50;
                    yAxes[yAxisID].max = 90;
                }
    ...
    async function cargarTodosLosGraficosIndividuales() {
        ...
        // 1. Destruimos cualquier gráfico individual que existiera previamente.
        Object.values(activeIndividualCharts).forEach(chart => chart.destroy());
        
        // 2. Vaciamos nuestro almacén de gráficos.
        activeIndividualCharts = {};
        
        // 3. Limpiamos completamente el contenedor HTML.
        grid.innerHTML = '';
        // ===== FIN DE LA SOLUCIÓN =====

        grid.innerHTML = '<p class="loading-message">Cargando datos de todos los sensores...</p>';

        // Usaremos el rango de la última semana por defecto
        const endTime = new Date();
        const startTime = new Date();
        startTime.setDate(endTime.getDate() - 7);

        try {
            // Hacemos una única llamada a la API que nos devuelve los datos agrupados por sensor y variable
            const apiUrl = `api/api_get_dashboard_data.php?obra=${configActualEscena.idEscena}&startTime=${startTime.toISOString()}&endTime=${endTime.toISOString()}`;
            const response = await fetch(apiUrl);
            if (response.status === 401) {
                alert("Tu sesión ha expirado. Por favor, inicia sesión de nuevo.");
                window.location.href = 'login.html';
            ...
            if (Object.keys(datosPorVariable).length === 0) {
                grid.innerHTML = '<p class="loading-message">No se encontraron datos para la última semana.</p>';
                return;
            }

            // Ahora, iteramos sobre los datos recibidos para crear un gráfico por cada sensor/variable
    ...
    // ===== NUEVA FUNCIÓN AUXILIAR PARA CREAR UNA TARJETA DE GRÁFICO INDIVIDUAL =====
    function createIndividualChartCard(sensorId, dataPoints) {
        ...
        const chartContainer = document.createElement('div');
        chartContainer.className = 'chart-container'; // Reutilizamos la clase de estilo
        ...
        const points = dataPoints.map(p => ({ x: p.timestamp, y: p.lectura }));
        
        const options = getUnifiedChartOptions(false); // getUnifiedChartOptions es una función que ya tienes
        
        // Aplicar ejes fijos
        if (sensorId.startsWith('CA')) {
            options.scales.y = { ...options.scales.y, min: -20, max: 20 };
        } else if (sensorId.startsWith('CL')) {
            options.scales.y = { ...options.scales.y, min: -10, max: 10 };
        }
        ...
        const newChartInstance = new Chart(ctx, {
            ...
            data: {
                datasets: [{
                    label: sensorId,
                    data: points,
                    borderColor: '#9cbbd3', // Usamos tu color principal
                    borderWidth: 1.5,
                    pointRadius: 0,
                    tension: 0.1
                }]
        ...
        activeIndividualCharts[chartId] = newChartInstance;
    }
